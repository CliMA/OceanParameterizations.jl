...
input_training_data = [FreeConvectionTrainingDataInput(i, T_scaling, wT_scaling) for i in input_training_data]
...
function nn_callback()
    μ_loss = mean(nn_loss(input, output) for (input, output) in training_data)
...
for opt in optimizers, e in 1:epochs, (i, mini_batch) in enumerate(data_loader)
    ...
    Flux.train!(nn_loss, NN_params, mini_batch, opt, cb=Flux.throttle(nn_callback, 2))
end

#####
##### Animate what the neural network has learned
#####

for Q in Qs_train
    filepath = "learned_heat_flux_initial_guess_Q$(Q)W.mp4"
    animate_learned_heat_flux(ds[Q], free_convection_neural_network, T_scaling, wT_scaling, grid_points=Nz, filepath=filepath, frameskip=5, fps=15)
end

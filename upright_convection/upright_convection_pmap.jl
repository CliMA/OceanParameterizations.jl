...
@everywhere @gen function kpp_proposal(trace)
...
@everywhere function do_inference(model, model_args, data; n_samples, max_iters=10n_samples)
...
@everywhere function do_inference_one_sample(model, model_args, data; iters)
    # Create a choice map that maps model addresses (:T, i, n)
    # to observed data T[i, n]. We leave the four KPP parameters
    # (:CSL, :CNL, :Cb_T, :CKE) unconstrained, because we want them
    # to be inferred.
...
@everywhere function do_inference_wrapper(n, model, args, data; iters)
    println("Sample $n")
    return do_inference_one_sample(model, args, data; iters=iters)
end

model_args = (ℂ, constants, N, L, Δt, times, T₀, FT, ∂T∂z)

samples = 10
traces = pmap(n -> do_inference_wrapper(free_convection_model, model_args,
                                        T_coarse_grained, iters=100),
              1:samples)

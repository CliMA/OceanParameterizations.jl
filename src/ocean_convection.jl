...
Create an animation of the variable `var` located in the NetCDF dataset `ds`. The coarse-grained version with `grid_points` will also be plotted. `xlabel` and `xlim` should be specified for the plot. A `filepath should also be specified`. If `filepath` already exists, no plot will be produced. `frameskip` > 1 can be used to skip some profiles to produce the animation more quickly. The output animation will be an mp4 with `fps` frames per second.
"""
...
"""
function training_data(ϕ; grid_points)
    Nz, Nt = size(ϕ)
    loc = location_z(ϕ)

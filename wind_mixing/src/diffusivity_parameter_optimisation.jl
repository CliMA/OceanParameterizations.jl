...
function DE_nonlocal(x, p, t, derivatives, scalings, constants, BCs)
    ν₀ = 1f-5
    ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr = p
...
function optimise_modified_pacanowski_philander_nonlocal(train_files, tsteps, timestepper, optimizers, maxiters, FILE_PATH;
    n_simulations, ν₁_conv = 1f-1, ν₁_en = 2f-2, ΔRi_conv=0.1f0, ΔRi_en=0.1f0, Riᶜ=0.25f0, Pr=1f0,
    train_gradient=true, gradient_scaling=5f-3,
    training_fractions=nothing)
    @info "Loading training data"
    𝒟 = WindMixing.data(train_files, scale_type=ZeroMeanUnitVarianceScaling, enforce_surface_fluxes=false)
    ...
    ν₁_conv_scaling = 1 / ν₁_conv
    ν₁_en_scaling = 1 / ν₁_en
    ΔRi_conv_scaling = 1 / ΔRi_conv
    ΔRi_en_scaling = 1 / ΔRi_en
    Riᶜ_scaling = 1 / Riᶜ
    Pr_scaling = 1 / Pr

    scale_parameter(parameter, scaling) = parameter * scaling

    unscale_parameter(parameter, scaling) = parameter / scaling
    ...
    function prepare_parameters()
        ...
        constants = (H=H, τ=τ, Nz=Nz, f=1f-4, α=2f-4, g=9.80665f0)
        scalings = (u=u_scaling, v=v_scaling, T=T_scaling, uw=uw_scaling, vw=vw_scaling, wT=wT_scaling, parameters=[ν₁_conv_scaling, ν₁_en_scaling, ΔRi_conv_scaling, ΔRi_en_scaling, Riᶜ_scaling, Pr_scaling])
        derivatives = (cell=Float32.(Dᶜ(Nz, 1 / Nz)), face=Float32.(Dᶠ(Nz, 1 / Nz)), Ri_nonlocal=D_Ri_nonlocal)
        parameters = [ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr]
        scaled_parameters = scale_parameter.(parameters, scalings.parameters)
    ...
    prob_NDEs = [ODEProblem((x, p, t) -> DE_nonlocal(x, p, t, derivatives, scalings, constants, BCs[i]), uvT₀s[i], tspan_train) for i in 1:n_simulations]
    ...
    prob_loss = OptimizationProblem(f_loss, scaled_parameters, lb=[0f0, 0f0, 0f0, 0f0, 0f0, 0f0], ub=[10f0, 10f0, 10f0, 10f0, 10f0, 10f0])
    ...
    for i in 1:length(optimizers)
        ...
        function cb(args...)
            if iter <= maxiters
                ...
                ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr = unscaled_parameters

                losses = args[3]
                profile_loss = losses.u + losses.v + losses.T
                gradient_loss = losses.∂u∂z + losses.∂v∂z + losses.∂T∂z
                total_loss = profile_loss + gradient_loss
                @info "ν₁_conv = $ν₁_conv, ν₁_en = $ν₁_en, ΔRi_conv = $ΔRi_conv, ΔRi_en = $ΔRi_en, Riᶜ = $Riᶜ, Pr = $Pr, loss = $(total_loss): profile = $profile_loss, gradient = $gradient_loss (u = $(losses.u), v = $(losses.v), T = $(losses.T)) , ∂u∂z = $(losses.∂u∂z), ∂v∂z = $(losses.∂v∂z), ∂T∂z = $(losses.∂T∂z)), optimizer $i/$(length(optimizers)), iteration = $iter/$maxiters"
                write_data_modified_pacanowski_philander_optimisation(FILE_PATH, losses, unscaled_parameters)
            end
            iter += 1
            false
    ...
    ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr = unscale_parameter.(scaled_parameters, scalings.parameters)

    @info "ν₁_conv = $ν₁_conv, ν₁_en = $ν₁_en, ΔRi_conv = $ΔRi_conv, ΔRi_en = $ΔRi_en, Riᶜ = $Riᶜ, Pr = $Pr, loss = $loss"
end
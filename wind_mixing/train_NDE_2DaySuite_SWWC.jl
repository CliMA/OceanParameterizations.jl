...
train_files = ["strong_wind_weak_cooling"]
...
function save_NDE_weights()
    ...
    bson(joinpath(PATH, "Output", "uw_NDE_weights_2DaySuite_SWWC_100.bson"), uw_NN_params)
SWWH_
    vw_NN_params = Dict(:weights => vw_weights)SWWH_
    bson(joinpath(PATH, "Output", "vw_NDE_weights_2DaySuite_SWWC_100.bson"), vw_NN_params)
SWWH_
    wT_NN_params = Dict(:weights => wT_weights)SWWH_
    bson(joinpath(PATH, "Output", "wT_NDE_weights_2DaySuite_SWWC_100.bson"), wT_NN_params)
endSWWH_

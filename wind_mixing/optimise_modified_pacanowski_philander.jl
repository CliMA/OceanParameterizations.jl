...
function optimise_modified_pacanowski_philander(𝒟, tsteps, timestepper, optimizers, maxiters; n_simulations)
    ...
    for i in 1:length(optimizers)
        ...
        function cb(args...)
            if iter <= maxiters
                @info "NDE, loss = $(args[2]), optimizer $i/$(length(optimizers)), iteration = $iter/$maxiters"
            end
            iter += 1
            false
        end

...
FILE_PATH = joinpath(OUTPUT_PATH, "NDE_training_modified_pacalowski_philander_1sim_-1e-3_smaller_learning_rate.jld2")
...
FILE_PATH_NN = joinpath(PATH, "extracted_training_output", "NDE_training_modified_pacalowski_philander_1sim_-1e-3_RMSProp_extracted.jld2")
...
train_optimizers = [[ADAM(5e-4), ADAM(2e-4), ADAM(1e-4), ADAM(5e-5)]]
timestepper = ROCK4()
...
function train(FILE_PATH, train_files, train_epochs, train_tranges, train_optimizers, uw_NN, vw_NN, wT_NN, 𝒟train, timestepper)
    ...
    for i in 1:length(train_epochs)
        ...
        uw_NN, vw_NN, wT_NN = train_NDE(uw_NN, vw_NN, wT_NN, 𝒟train, train_tranges[i], timestepper, train_optimizers[i], train_epochs[i], FILE_PATH, i, n_simulations=1, maxiters=300, modified_pacalowski_philander=true, ν₀=1f-4, ν₋=1f-1, ΔRi=1f0, convective_adjustment=false)

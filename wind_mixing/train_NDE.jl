...
FILE_PATH_NN = joinpath(PATH, "extracted_training_output", "NDE_training_modified_pacalowski_philander_1sim_-1e-3_2_extracted.jld2")
...
train_epochs = [10, 100]
train_tranges = [1:10:500, 1:20:1153]
...
train_optimizers = [[RMSProp()], [RMSProp()]]
timestepper = ROCK4()
...
function train(FILE_PATH, train_files, train_epochs, train_tranges, train_optimizers, uw_NN, vw_NN, wT_NN, 𝒟train, timestepper)
    ...
    for i in 1:length(train_epochs)
        ...
        uw_NN, vw_NN, wT_NN = train_NDE(uw_NN, vw_NN, wT_NN, 𝒟train, train_tranges[i], timestepper, train_optimizers[i], train_epochs[i], FILE_PATH, i, n_simulations=1, maxiters=5, modified_pacalowski_philander=true, ν₀=1f-4, ν₋=1f-1, ΔRi=1f0, convective_adjustment=false)

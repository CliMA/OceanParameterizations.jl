...
FILE_NAME = "NDE_18sim_windcooling_windheating_18simBFGST0.8nogradnonlocal_divide1f5_gradient_smallNN_leakyrelu_layers_1_rate_2e-4_T0.8_2e-4"
...
ν₋ = train_parameters["ν₁_conv"]
ΔRi = train_parameters["ΔRi_conv"]
...
ax = fig[1,1] = CairoMakie.Axis(fig, yscale=log10, xlabel=L"Ri$ $", ylabel=L"Local diffusivity (m$^2$ s$^{-1}$)")
...
text!(ax, -2, 1e-2, text="Convection-driven \n mixing", fontsize=50, font=:bold, align=(:center, :center))
text!(ax, 3, 1e-2, text=" Shear-driven mixing", fontsize=50, font=:bold, align=(:center, :center))

axislegend(ax, [ν_line, ν_T_line], ["Momentum Diffusivity", "Temperature Diffusivity"])
display(fig)
##
save("plots/mpp_diffusivity_nonlocal.pdf", fig, px_per_unit=4, pt_per_unit=4)
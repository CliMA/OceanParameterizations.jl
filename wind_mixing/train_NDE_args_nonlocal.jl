...
elseif params_type == "18simBFGST0.8nogradnonu0"
  ...
  ν₀ = 1f-5
elseif params_type == "18simBFGST0.8nogradnonlocal"
  PARAMETERS_PATH = joinpath(EXTRACTED_OUTPUT_PATH, "parameter_optimisation_nonlocal_18sim_windcooling_windheating_5params_BFGS_T0.8_var_nograd_new_extracted.jld2")  
  parameters_file = jldopen(PARAMETERS_PATH)
  mpp_parameters = parameters_file["parameters"]
  close(parameters_file)  
  ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr = mpp_parameters
  ν₀ = 1f-5
elseif params_type == "18simLBFGST0.8nogradnonlocal"
  PARAMETERS_PATH = joinpath(EXTRACTED_OUTPUT_PATH, "parameter_optimisation_nonlocal_18sim_windcooling_windheating_5params_LBFGS_T0.8_var_nograd_new_extracted.jld2")  
  parameters_file = jldopen(PARAMETERS_PATH)
  mpp_parameters = parameters_file["parameters"]
  close(parameters_file)  
  ν₁_conv, ν₁_en, ΔRi_conv, ΔRi_en, Riᶜ, Pr = mpp_parameters
...
function train(FILE_PATH, train_files, train_epochs, train_tranges, train_parameters, train_optimizers, train_iterations, uw_NN, vw_NN, wT_NN, timestepper)
    ...
    for i in 1:length(train_epochs)
        ...
        if train_parameters["modified_pacanowski_philander"]
            uw_NN, vw_NN, wT_NN = train_NDE_nonlocal(uw_NN, vw_NN, wT_NN, train_files, train_tranges[i], timestepper, train_optimizers[i], train_epochs[i], FILE_PATH, i, 
...
animate_training_results_nonlocal(test_files, FILE_NAME,

...
train_files = ["strong_wind_no_coriolis"]
...
function save_NDE_weights()
    ...
    bson(joinpath(PATH, "Output", "uw_NDE_weights_2DaySuite_SWNR_100.bson"), uw_NN_params)

    vw_NN_params = Dict(:weights => vw_weights)
    bson(joinpath(PATH, "Output", "vw_NDE_weights_2DaySuite_SWNR_100.bson"), vw_NN_params)

    wT_NN_params = Dict(:weights => wT_weights)
    bson(joinpath(PATH, "Output", "wT_NDE_weights_2DaySuite_SWNR_100.bson"), wT_NN_params)
end
